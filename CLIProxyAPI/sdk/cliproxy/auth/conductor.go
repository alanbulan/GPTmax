...
const (
	...
	refreshMaxConcurrency  = 2
	refreshPendingBackoff  = time.Minute
	refreshFailureBackoff  = 5 * time.Minute
	quotaBackoffBase       = time.Second
	quotaBackoffMax        = 30 * time.Minute
	refreshBatchSmallPool  = 64
	refreshBatchMediumPool = 24
	refreshBatchLargePool  = 12
	refreshBatchHugePool   = 6
)
...
func clearAuthStateOnSuccess(auth *Auth, now time.Time) {
	...
	auth.Metadata["last_checked_at"] = now.Format(time.RFC3339)
	auth.Metadata["failure_count"] = 0
...
func applyAuthFailureState(auth *Auth, resultErr *Error, retryAfter *time.Duration, now time.Time) {
	...
	auth.UpdatedAt = now
	auth.Metadata["last_checked_at"] = now.Format(time.RFC3339)
	auth.Metadata["failure_count"] = metadataInt(auth.Metadata, "failure_count") + 1
	if resultErr != nil {
	...
	if !auth.NextRetryAfter.IsZero() {
		auth.Metadata["next_retry_after"] = auth.NextRetryAfter.Format(time.RFC3339)
	} else {
		delete(auth.Metadata, "next_retry_after")
	}
}
...
func shouldBypassSchedulerForProviders(providers []string) bool {
	if len(providers) == 0 {
		return false
	}
	for _, provider := range providers {
...
func (m *Manager) pickNextLegacy(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, error) {
	...
	if strings.EqualFold(strings.TrimSpace(provider), "codex") && pinnedAuthID == "" {
		sort.Slice(candidates, func(i, j int) bool {
			left := m.authInFlight[candidates[i].ID]
			right := m.authInFlight[candidates[j].ID]
			if left == right {
...
func (m *Manager) pickNextMixedLegacy(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	...
	if shouldBypassSchedulerForProviders(providers) && pinnedAuthID == "" {
		sort.Slice(candidates, func(i, j int) bool {
			left := m.authInFlight[candidates[i].ID]
			right := m.authInFlight[candidates[j].ID]
			if left == right {
...
func (m *Manager) pickNextMixed(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, ProviderExecutor, string, error) {
	if shouldBypassSchedulerForProviders(providers) {
...
func (m *Manager) beginAuthUse(authID string) {
	if m == nil || strings.TrimSpace(authID) == "" {
		return
	}
	m.mu.Lock()
	if m.authInFlight == nil {
		m.authInFlight = make(map[string]int)
	}
	m.authInFlight[authID]++
	m.mu.Unlock()
}

func (m *Manager) endAuthUse(authID string) {
	if m == nil || strings.TrimSpace(authID) == "" {
		return
	}
	m.mu.Lock()
	if current := m.authInFlight[authID]; current <= 1 {
...
func (m *Manager) checkRefreshes(ctx context.Context) {
	...
	batchLimit := autoRefreshBatchLimit(len(snapshot))
	if batchLimit <= 0 {
		return
	}
	start := int(m.refreshCursor.Add(uint64(batchLimit)) - uint64(batchLimit))
	start %= len(snapshot)
	scheduled := 0
	for i := 0; i < len(snapshot) && scheduled < batchLimit; i++ {
		a := snapshot[(start+i)%len(snapshot)]
		typ, _ := a.AccountInfo()
		if typ != "api_key" {
	...
	if scheduled > 0 {
		log.Debugf("core auth auto-refresh scheduled %d/%d candidates", scheduled, len(snapshot))
	}
}
...
func (m *Manager) shouldRefresh(a *Auth, now time.Time) bool {
	...
	if shouldSkipRefreshByHealth(a, now) {
		return false
	}
...
func autoRefreshBatchLimit(total int) int {
	switch {
	case total <= 0:
		return 0
	case total <= 500:
		return refreshBatchSmallPool
	case total <= 5000:
		return refreshBatchMediumPool
	case total <= 20000:
		return refreshBatchLargePool
	default:
		return refreshBatchHugePool
	}
}

func shouldSkipRefreshByHealth(a *Auth, now time.Time) bool {
	if a == nil {
		return true
	}
	if metadataTruthy(a.Metadata, "token_invalid") {
		return true
	}
	if strings.EqualFold(strings.TrimSpace(a.StatusMessage), "token_invalid") {
		return true
	}
	if metadataTruthy(a.Metadata, "region_blocked") && (a.NextRetryAfter.IsZero() || a.NextRetryAfter.After(now)) {
		return true
	}
	if metadataInt(a.Metadata, "failure_count") >= 3 && (a.NextRetryAfter.IsZero() || a.NextRetryAfter.After(now)) {
		return true
	}
	return false
}
...
func (m *Manager) refreshAuth(ctx context.Context, id string) {
	...
	if err != nil {
		...
		if current := m.auths[id]; current != nil {
			...
			current.Metadata["last_checked_at"] = now.Format(time.RFC3339)
			current.Metadata["failure_count"] = metadataInt(current.Metadata, "failure_count") + 1
			if isPermanentRefreshFailure(err) {
				current.Disabled = true
				current.Status = StatusDisabled
				current.Unavailable = true
				current.StatusMessage = "token_invalid"
				current.NextRefreshAfter = now.Add(365 * 24 * time.Hour)
				current.NextRetryAfter = now.Add(365 * 24 * time.Hour)
				if current.Metadata == nil {
					current.Metadata = make(map[string]any)
				}
				current.Metadata["token_invalid"] = true
				current.Metadata["token_invalid_reason"] = err.Error()
				current.Metadata["token_invalid_at"] = now.Format(time.RFC3339)
			} else {
				current.NextRefreshAfter = now.Add(refreshFailureBackoff)
			}
			if !current.NextRetryAfter.IsZero() {
				current.Metadata["next_retry_after"] = current.NextRetryAfter.Format(time.RFC3339)
			} else {
				delete(current.Metadata, "next_retry_after")
			}
			m.auths[id] = current
			if m.scheduler != nil {
	...
	updated.Metadata["last_checked_at"] = now.Format(time.RFC3339)
	updated.Metadata["failure_count"] = 0
	delete(updated.Metadata, "next_retry_after")
