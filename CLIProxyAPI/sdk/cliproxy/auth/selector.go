...
func isAuthBlockedForModel(auth *Auth, model string, now time.Time) (bool, blockReason, time.Time) {
	...
	if auth.Metadata != nil {
		if raw, ok := auth.Metadata["token_invalid"]; ok {
			switch v := raw.(type) {
			case bool:
				if v {
		...
		if raw, ok := auth.Metadata["region_blocked"]; ok {
			switch v := raw.(type) {
			case bool:
				if v {
		...
		if raw, ok := auth.Metadata["failure_count"]; ok {
			switch v := raw.(type) {
			case int:
				if v >= 5 && auth.NextRetryAfter.After(now) {
					return true, blockReasonOther, auth.NextRetryAfter
				}
			case float64:
				if int(v) >= 5 && auth.NextRetryAfter.After(now) {
					return true, blockReasonOther, auth.NextRetryAfter
				}
			}
		}
