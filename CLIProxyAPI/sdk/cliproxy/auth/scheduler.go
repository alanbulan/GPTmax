...
func (s *authScheduler) pickSingle(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, error) {
	...
	scanLimit := selectionScanLimit(pinnedAuthID, tried)
	...
	if picked := shard.pickReadyLocked(preferWebsocket, s.strategy, predicate, scanLimit); picked != nil {
		return picked, nil
	}
...
func (s *authScheduler) pickMixed(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}) (*Auth, string, error) {
	...
	if pinnedAuthID != "" {
		...
		if picked := shard.pickReadyLocked(false, s.strategy, predicate, scanLimit); picked != nil {
	...
	for providerIndex, providerKey := range normalized {
		...
		priorityReady, okPriority := shard.highestReadyPriorityLocked(false, predicate, scanLimit)
		if !okPriority {
			continue
		}
	...
	if s.strategy == schedulerStrategyFillFirst {
		for providerIndex, providerKey := range normalized {
			...
			picked := shard.pickReadyAtPriorityLocked(false, bestPriority, s.strategy, predicate, scanLimit)
			if picked != nil {
				return picked, providerKey, nil
			}
		}
	...
	for offset := 0; offset < len(normalized); offset++ {
		...
		picked := shard.pickReadyAtPriorityLocked(false, bestPriority, schedulerStrategyRoundRobin, predicate, scanLimit)
		if picked == nil {
			continue
		}
...
func selectionScanLimit(pinnedAuthID string, tried map[string]struct{}) int {
	if strings.TrimSpace(pinnedAuthID) != "" {
		return 0
	}
	limit := hotPathCandidateWindow + len(tried)
	if limit < hotPathCandidateWindow {
		return hotPathCandidateWindow
	}
	return limit
}
...
func (m *modelScheduler) pickReadyLocked(preferWebsocket bool, strategy schedulerStrategy, predicate func(*scheduledAuth) bool, scanLimit int) *Auth {
	if m == nil {
		return nil
	}
	m.promoteExpiredLocked(time.Now())
	priorityReady, okPriority := m.highestReadyPriorityLocked(preferWebsocket, predicate, scanLimit)
	if !okPriority {
		return nil
	}
	return m.pickReadyAtPriorityLocked(preferWebsocket, priorityReady, strategy, predicate, scanLimit)
}

// highestReadyPriorityLocked returns the highest priority bucket that still has a matching ready auth.
// The caller must ensure expired entries are already promoted when needed.
func (m *modelScheduler) highestReadyPriorityLocked(preferWebsocket bool, predicate func(*scheduledAuth) bool, scanLimit int) (int, bool) {
	if m == nil {
		return 0, false
	}
	for _, priority := range m.priorityOrder {
		...
		if view.pickFirst(predicate, scanLimit) != nil {
...
// The caller must ensure expired entries are already promoted when needed.
func (m *modelScheduler) pickReadyAtPriorityLocked(preferWebsocket bool, priority int, strategy schedulerStrategy, predicate func(*scheduledAuth) bool, scanLimit int) *Auth {
	if m == nil {
		return nil
	}
	bucket := m.readyByPriority[priority]
	...
	if strategy == schedulerStrategyFillFirst {
		picked = view.pickFirst(predicate, scanLimit)
	} else {
		picked = view.pickRoundRobin(predicate, scanLimit)
	}
...
func (v *readyView) pickFirst(predicate func(*scheduledAuth) bool, scanLimit int) *scheduledAuth {
	for index, entry := range v.flat {
		if scanLimit > 0 && index >= scanLimit {
			break
		}
		if predicate == nil || predicate(entry) {
			return entry
...
func (v *readyView) pickRoundRobin(predicate func(*scheduledAuth) bool, scanLimit int) *scheduledAuth {
	if len(v.parentOrder) > 1 && len(v.children) > 0 {
		return v.pickGroupedRoundRobin(predicate, scanLimit)
	}
	if len(v.flat) == 0 {
		return nil
	}
	...
	for offset := 0; offset < len(v.flat); offset++ {
		if scanLimit > 0 && offset >= scanLimit {
			break
		}
...
func (v *readyView) pickGroupedRoundRobin(predicate func(*scheduledAuth) bool, scanLimit int) *scheduledAuth {
	start := 0
