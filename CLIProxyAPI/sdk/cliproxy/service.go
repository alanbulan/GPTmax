...
const (
	modelRefreshBatchSize  = 250
	modelRefreshBatchPause = 10 * time.Millisecond
)
...
type Service struct {
	...
	// modelRefreshWake wakes the async model re-registration worker.
	modelRefreshWake chan struct{}

	// modelRefreshStop cancels the async model re-registration worker.
	modelRefreshStop context.CancelFunc

	// modelRefreshMu guards pending provider refresh requests.
	modelRefreshMu sync.Mutex

	// modelRefreshPending stores providers awaiting re-registration.
	modelRefreshPending map[string]struct{}
...
func (s *Service) ensureModelRefreshQueue(ctx context.Context) {
	if s == nil {
		return
	}
	if s.modelRefreshWake == nil {
		s.modelRefreshWake = make(chan struct{}, 1)
	}
	if s.modelRefreshPending == nil {
		s.modelRefreshPending = make(map[string]struct{})
	}
	if s.modelRefreshStop != nil {
		return
	}
	queueCtx, cancel := context.WithCancel(ctx)
	s.modelRefreshStop = cancel
	go s.consumeModelRefreshes(queueCtx)
}

func (s *Service) enqueueModelRefresh(providers []string) {
	...
	for _, provider := range providers {
		...
		s.modelRefreshPending[key] = struct{}{}
	}
	s.modelRefreshMu.Unlock()
	if s.modelRefreshWake != nil {
		select {
		case s.modelRefreshWake <- struct{}{}:
		default:
		}
	}
}

func (s *Service) takePendingModelRefreshProviders() []string {
	if s == nil {
		return nil
	}
	s.modelRefreshMu.Lock()
	defer s.modelRefreshMu.Unlock()
	if len(s.modelRefreshPending) == 0 {
		return nil
	}
	providers := make([]string, 0, len(s.modelRefreshPending))
	for provider := range s.modelRefreshPending {
...
func (s *Service) consumeModelRefreshes(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			return
		case <-s.modelRefreshWake:
			for {
				providers := s.takePendingModelRefreshProviders()
				if len(providers) == 0 {
					break
				}
				s.processModelRefresh(ctx, providers)
			}
		}
	}
}

func (s *Service) processModelRefresh(ctx context.Context, changedProviders []string) {
	...
	refreshed := 0
	startedAt := time.Now()
	for offset := 0; offset < len(candidates); offset += modelRefreshBatchSize {
		if ctx != nil && ctx.Err() != nil {
			return
		}
		end := offset + modelRefreshBatchSize
		if end > len(candidates) {
			end = len(candidates)
		}
		for _, auth := range candidates[offset:end] {
			if s.refreshModelRegistrationForAuth(auth) {
				refreshed++
			}
		}
		if end < len(candidates) {
			time.Sleep(modelRefreshBatchPause)
		}
	}
	log.Infof("re-registered models for %d auth(s) due to model catalog changes: %v (async, %s)", refreshed, changedProviders, time.Since(startedAt).Round(time.Millisecond))
}
...
func (s *Service) Run(ctx context.Context) error {
	...
	registry.SetModelRefreshCallback(func(changedProviders []string) {
		...
		s.enqueueModelRefresh(changedProviders)
	})
