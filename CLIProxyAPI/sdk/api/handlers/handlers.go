...
const (
	debugPinnedAuthHeader   = "X-CPA-Pinned-Auth-ID"
	debugSelectedAuthHeader = "X-CPA-Selected-Auth-ID"
	debugAuthTraceHeader    = "X-CPA-Debug-Auth"
)
...
func (h *BaseAPIHandler) GetContextWithCancel(handler interfaces.APIHandler, c *gin.Context, ctx context.Context) (context.Context, APIHandlerCancelFunc) {
	...
	if c != nil {
		pinnedAuthID := strings.TrimSpace(c.GetHeader(debugPinnedAuthHeader))
		if pinnedAuthID != "" {
			newCtx = WithPinnedAuthID(newCtx, pinnedAuthID)
		}
		if pinnedAuthID != "" || strings.EqualFold(strings.TrimSpace(c.GetHeader(debugAuthTraceHeader)), "1") {
			newCtx = WithSelectedAuthIDCallback(newCtx, func(authID string) {
				if c.Writer != nil && strings.TrimSpace(authID) != "" {
					c.Writer.Header().Set(debugSelectedAuthHeader, strings.TrimSpace(authID))
				}
			})
		}
	}
