...
func ConvertCodexResponseToOpenAINonStream(_ context.Context, _ string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, _ *any) string {
	...
	if !responseResult.Exists() {
		// Non-streaming Codex Responses API returns the response object directly
		// (e.g. object=response / response.compaction) instead of wrapping it in
		// a response.completed SSE envelope.
		if objectType := rootResult.Get("object").String(); strings.HasPrefix(objectType, "response.") || objectType == "response" {
