...
const initialReloadAsyncThreshold = 1000
...
func (w *Watcher) shouldAsyncInitialReload() bool {
	if w == nil {
		return false
	}
	...
	for _, entry := range entries {
		...
		if strings.HasSuffix(strings.ToLower(entry.Name()), ".json") {
			jsonCount++
			if jsonCount >= initialReloadAsyncThreshold {
				return true
			}
		}
	}
