...
type authFilesLookupRequest struct {
	Names   []string `json:"names"`
	Type    string   `json:"type"`
	Compact bool     `json:"compact"`
}

type authFileListSummary struct {
	Name       string
	Type       string
	Provider   string
	SearchText string
...
func metadataBoolValue(meta map[string]any, keys ...string) bool {
	if len(meta) == 0 {
		return false
	}
...
func metadataIntValue(meta map[string]any, keys ...string) (int, bool) {
	if len(meta) == 0 {
		return 0, false
	}
	for _, key := range keys {
...
func metadataTimeValue(meta map[string]any, keys ...string) (time.Time, bool) {
	if len(meta) == 0 {
		return time.Time{}, false
	}
	for _, key := range keys {
		...
		if ts, ok := parseLastRefreshValue(raw); ok {
			return ts, true
		}
	}
	return time.Time{}, false
...
func parseAuthFilesListQuery(c *gin.Context) authFilesListQuery {
	page, hasPage := parsePositiveQueryInt(c, "page")
	pageSize, hasPageSize := parsePositiveQueryInt(c, "page_size", "pageSize")
	typeFilter := normalizeAuthFileType(c.Query("type"))
	search := strings.ToLower(strings.TrimSpace(c.Query("search")))
	compact := queryTruthy(c, "compact")
	paginated := hasPage || hasPageSize || typeFilter != "" || search != ""
...
func parsePositiveQueryInt(c *gin.Context, keys ...string) (int, bool) {
	for _, key := range keys {
...
func normalizeAuthFileType(raw string) string {
...
func (q authFilesListQuery) matches(summary authFileListSummary) bool {
	if q.Type != "" && summary.Type != q.Type {
		return false
	}
	if q.Search != "" && !strings.Contains(summary.SearchText, q.Search) {
		return false
	}
	return true
}

func authFilesPagination(total, page, pageSize int) (start int, end int, currentPage int, totalPages int) {
	if page <= 0 {
		page = 1
	}
	if pageSize <= 0 {
...
func newAuthFilesResponse(files []gin.H, matchedTotal int, typeCounts map[string]int, query authFilesListQuery) gin.H {
	if typeCounts == nil {
	...
	response := gin.H{
		"files":       files,
		"total":       matchedTotal,
		"all_total":   typeCounts["all"],
		"type_counts": typeCounts,
	}
	if query.Paginated {
		_, _, page, totalPages := authFilesPagination(matchedTotal, query.Page, query.PageSize)
		response["page"] = page
...
func normalizeTypeCounts(typeCounts map[string]int) map[string]int {
	out := make(map[string]int, len(typeCounts)+1)
	out["all"] = typeCounts["all"]
...
func authFileSortName(name string) string {
	return strings.ToLower(strings.TrimSpace(name))
}

func mapToGinH(entry map[string]any) gin.H {
	if len(entry) == 0 {
		return gin.H{}
	}
	out := make(gin.H, len(entry))
...
func (h *Handler) ListAuthFiles(c *gin.Context) {
	query := parseAuthFilesListQuery(c)
	if h == nil {
		c.JSON(500, gin.H{"error": "handler not initialized"})
		return
	}
	if h.authMetaStore != nil && query.Compact {
		result, err := h.authMetaStore.List(c.Request.Context(), metastore.AuthMetaListQuery{
			Page:     query.Page,
		...
		if err == nil && result != nil {
			...
			for _, entry := range result.Entries {
				files = append(files, mapToGinH(entry))
			}
			c.JSON(200, newAuthFilesResponse(files, result.Total, normalizeTypeCounts(result.TypeCounts), query))
			return
		}
		if err != nil {
			log.WithError(err).Warn("auth meta store list failed, falling back to auth manager")
		}
	}
	...
	items := make([]managedAuthFile, 0, len(auths))
	typeCounts := map[string]int{"all": 0}
	for _, auth := range auths {
		summary, ok := h.authFileSummary(auth)
		if !ok {
			continue
		}
		typeCounts["all"]++
		...
		items = append(items, managedAuthFile{auth: auth, summary: summary})
	}
	sort.Slice(items, func(i, j int) bool {
		nameI := authFileSortName(items[i].summary.Name)
		nameJ := authFileSortName(items[j].summary.Name)
		if nameI == nameJ {
			return strings.ToLower(strings.TrimSpace(items[i].auth.ID)) < strings.ToLower(strings.TrimSpace(items[j].auth.ID))
		}
		return nameI < nameJ
	})

	matchedTotal := len(items)
	if query.Paginated {
		start, end, _, _ := authFilesPagination(matchedTotal, query.Page, query.PageSize)
		items = items[start:end]
	}

	files := make([]gin.H, 0, len(items))
	for _, item := range items {
		if entry := h.buildAuthFileEntry(item.auth, query.Compact); entry != nil {
			files = append(files, entry)
		}
	}

	c.JSON(200, newAuthFilesResponse(files, matchedTotal, normalizeTypeCounts(typeCounts), query))
}

func (h *Handler) LookupAuthFiles(c *gin.Context) {
	if h == nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "handler not initialized"})
		return
	}

	var req authFilesLookupRequest
	...
	for _, raw := range req.Names {
		...
		key := normalizeLookupName(trimmed)
		if trimmed == "" || key == "" {
			continue
		}
		if _, ok := seen[key]; ok {
	...
	typeFilter := normalizeAuthFileType(req.Type)
	if h.authMetaStore != nil {
		lookup, err := h.authMetaStore.Lookup(c.Request.Context(), normalizedNames, typeFilter)
		if err == nil {
			items := make([]gin.H, 0, len(normalizedNames))
			for _, name := range normalizedNames {
				...
				if entry, ok := lookup[key]; ok {
					item["exists"] = true
					item["entry"] = mapToGinH(entry)
					if actualName, okName := entry["name"].(string); okName && strings.TrimSpace(actualName) != "" {
		...
		log.WithError(err).Warn("auth meta store lookup failed, falling back to auth manager")
	}
	if h.authManager == nil {
		h.lookupAuthFilesFromDisk(c, normalizedNames, typeFilter)
		return
	}

	wanted := make(map[string]struct{}, len(normalizedNames))
	for _, name := range normalizedNames {
		wanted[normalizeLookupName(name)] = struct{}{}
	}
	found := make(map[string]gin.H, len(normalizedNames))
	for _, auth := range h.authManager.List() {
		...
		key := normalizeLookupName(summary.Name)
		...
		if entry := h.buildAuthFileEntry(auth, true); entry != nil {
			found[key] = entry
		}
	}
	...
	for _, name := range normalizedNames {
		...
		if entry, ok := found[key]; ok {
			item["exists"] = true
			item["entry"] = entry
			if actualName, ok := entry["name"].(string); ok && strings.TrimSpace(actualName) != "" {
				item["name"] = actualName
			}
		}
		items = append(items, item)
...
func (h *Handler) listAuthFilesFromDisk(c *gin.Context, query authFilesListQuery) {
	entries, err := os.ReadDir(h.cfg.AuthDir)
	if err != nil {
		c.JSON(500, gin.H{"error": fmt.Sprintf("failed to read auth dir: %v", err)})
		return
	}
	files := make([]diskAuthFile, 0)
	typeCounts := map[string]int{"all": 0}
	for _, e := range entries {
		...
		if info, errInfo := e.Info(); errInfo == nil {
			...
			summary := authFileSummaryFromEntry(fileData)
			typeCounts["all"]++
			typeCounts[summary.Type]++
			if !query.matches(summary) {
				continue
			}
			files = append(files, diskAuthFile{entry: fileData, summary: summary})
		}
	}
	sort.Slice(files, func(i, j int) bool {
		nameI := authFileSortName(files[i].summary.Name)
		nameJ := authFileSortName(files[j].summary.Name)
		return nameI < nameJ
	})
	matchedTotal := len(files)
	if query.Paginated {
		start, end, _, _ := authFilesPagination(matchedTotal, query.Page, query.PageSize)
		files = files[start:end]
	}
	pageEntries := make([]gin.H, 0, len(files))
	for _, item := range files {
		pageEntries = append(pageEntries, item.entry)
	}
	c.JSON(200, newAuthFilesResponse(pageEntries, matchedTotal, normalizeTypeCounts(typeCounts), query))
}

func (h *Handler) lookupAuthFilesFromDisk(c *gin.Context, names []string, typeFilter string) {
	...
	for _, rawName := range names {
		...
		candidates := []string{filepath.Base(strings.TrimSpace(rawName))}
		if !strings.HasSuffix(strings.ToLower(candidates[0]), ".json") {
			candidates = append(candidates, candidates[0]+".json")
		}
		for _, candidate := range candidates {
			if candidate == "" {
				continue
			}
			full := filepath.Join(h.cfg.AuthDir, candidate)
			...
			if data, errRead := os.ReadFile(full); errRead == nil {
				if typeValue := gjson.GetBytes(data, "type").String(); typeValue != "" {
					entry["type"] = typeValue
					entry["provider"] = typeValue
				}
				if emailValue := gjson.GetBytes(data, "email").String(); emailValue != "" {
					entry["email"] = emailValue
				}
			}
			summary := authFileSummaryFromEntry(entry)
			if typeFilter != "" && summary.Type != typeFilter {
				break
			}
			item["exists"] = true
...
func (h *Handler) buildAuthFileEntry(auth *coreauth.Auth, compact bool) gin.H {
	if auth == nil {
		return nil
	}
	auth.EnsureIndex()
	...
	if path == "" && !runtimeOnly && h != nil && h.cfg != nil {
	...
	if compact {
		...
		if ts, ok := metadataTimeValue(auth.Metadata, "last_checked_at", "lastCheckedAt"); ok {
			entry["last_checked_at"] = ts
		}
		if !auth.NextRetryAfter.IsZero() {
			entry["next_retry_after"] = auth.NextRetryAfter
		} else if ts, ok := metadataTimeValue(auth.Metadata, "next_retry_after", "nextRetryAfter"); ok {
			entry["next_retry_after"] = ts
		}
		if failureCount, ok := metadataIntValue(auth.Metadata, "failure_count", "failureCount"); ok {
			entry["failure_count"] = failureCount
		}
		if metadataBoolValue(auth.Metadata, "token_invalid") {
			entry["token_invalid"] = true
		}
		if metadataBoolValue(auth.Metadata, "region_blocked", "regionBlocked") {
		...
		if claims := extractCodexIDTokenClaims(auth); claims != nil {
			if chatgptAccountID, ok := claims["chatgpt_account_id"]; ok {
				entry["id_token"] = gin.H{"chatgpt_account_id": chatgptAccountID}
			}
		}
		return entry
	}
...
func (h *Handler) authFileSummary(auth *coreauth.Auth) (authFileListSummary, bool) {
	if auth == nil {
		return authFileListSummary{}, false
	}
	auth.EnsureIndex()
	...
	fileType := normalizeAuthFileType(auth.Provider)
	if fileType == "" {
		fileType = "unknown"
	}
...
func authFileSummaryFromEntry(entry gin.H) authFileListSummary {
	name, _ := entry["name"].(string)
	provider, _ := entry["provider"].(string)
	if provider == "" {
		if rawType, ok := entry["type"].(string); ok {
			provider = rawType
		}
	}
	email, _ := entry["email"].(string)
	fileType := normalizeAuthFileType(provider)
	if fileType == "" {
		fileType = "unknown"
	}
	return authFileListSummary{
	...
	if all := c.Query("all"); all == "true" || all == "1" || all == "*" {
		...
		if typeFilter != "" || search != "" {
			...
			for _, auth := range h.authManager.List() {
				...
				if search != "" && !strings.Contains(summary.SearchText, search) {
					continue
				}
				...
				if errRemove := os.Remove(targetPath); errRemove == nil {
					...
					if errDel := h.deleteTokenRecord(ctx, targetPath); errDel != nil {
