...
export function AuthFilesPage() {
  ...
  const handleHeaderRefresh = useCallback(async () => {
    await Promise.all([
      loadFiles({ page, pageSize, filter, search }),
      refreshKeyStats(),
      loadExcluded(),
      loadModelAlias()
    ]);
  }, [filter, loadFiles, loadExcluded, loadModelAlias, page, pageSize, refreshKeyStats, search]);
  ...
  useEffect(() => {
    if (!isCurrentLayer) return;
    void loadFiles({ page, pageSize, filter, search });
  }, [filter, isCurrentLayer, loadFiles, page, pageSize, search]);
  ...
  const existingTypes = useMemo(() => {
    const types = Object.keys(authFileTypeCounts)
      ...
      {existingTypes.map((type) => {
        ...
        return (
          ...
          >
            ...
            <span className={styles.filterTagCount}>{authFileTypeCounts[type] ?? 0}</span>
          </button>
        );
      })}
    </div>
  ...
  const titleNode = (
    <div className={styles.titleWrapper}>
      ...
      {allTotal > 0 && <span className={styles.countBadge}>{allTotal}</span>}
    </div>
  );
